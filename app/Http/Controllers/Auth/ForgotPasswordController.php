...
use App\EmailCheck;
use App\Http\Requests\EmailCheckNumber;
...
class ForgotPasswordController extends ApiController
{
    ...
    public function sendResetLinkEmail(Request $request)
    {
        ...
        if(!$user == NULL){
            ...
            //현재 보낸 시간보다 오래된거는 삭제
            EmailCheck::where('email', $data->email)->where('created_at', '<', $data->created_at)->where('type', EmailCheck::PW)->delete();
    ...
    public function emailCheckNumber(Request $request)
    {
        ...
        $DB_Data = EmailCheck::where('email', $request->email)->where('type', EmailCheck::PW)->latest('created_at')->first();
        ...
        try {
            ...
            EmailCheck::where('token', $data->token)->where('type', EmailCheck::PW)->delete();

            return $this->ApiResponse('stable', ['인증되었습니다.', 'Certified.'], ['token' => $data->token,], 200);
